...
- **Two-box discount is server-computed and baked into the row amounts.** Grouped by **spec only** (`5`/`10`/`20`), *not* by product — boxes of the same spec across different products combine. Every 2 boxes of a spec deducts `PAIR_DISCOUNT` (100), cumulative — `floor(specTotalQty/2)*100`. The per-row `金額` is distributed in two steps so each row reads naturally and the rows still sum to the discounted total: (1) each item is discounted for its own pairs — `floor(qty/2)*100`; (2) leftover odd boxes of the same spec are paired across products, each cross-pair deducting another 100 (booked on the higher-priced row of the pair). The two steps sum to `floor(specTotalQty/2)*100` by identity. There is no separate discount column (don't add one; it breaks the positional invariant). The frontend mirrors the same grouping for display only (`computeDiscount` / `PAIR_DISCOUNT` in `script.js`); keep the constant and the per-spec grouping in sync. Since one order has one address, the discount only ever combines boxes going to that address.
