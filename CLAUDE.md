...
- **Two-box discount is server-computed and baked into the row amount.** Same product + same spec, every 2 boxes deducts `PAIR_DISCOUNT` (100), cumulative — `floor(qty/2)*100`. The discounted net is written to the `金額` column per item (there is no separate discount column; don't add one — it breaks the positional invariant). The frontend mirrors the same formula for display only (`PAIR_DISCOUNT` in `script.js`); keep the two constants in sync. Because the discount is per single item row, it only ever combines boxes going to the one order address.
